...
A few months ago, I wrote a [post](https://robertohuertas.com/2023/01/09/shuttle-rust-backend-deployment/) about [Shuttle](https://www.shuttle.rs/), where I explained how ridiculously easy it is to deploy a Rust backend to the cloud by using their [CLI tool](https://docs.shuttle.rs/introduction/quick-start).
...
As you can see in the code above, we are using the [Shuttle Secrets](https://docs.shuttle.rs/resources/shuttle-secrets) crate to get the [Datadog](https://datadoghq.com) API key, the tags and the log level.
