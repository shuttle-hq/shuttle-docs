...
[installed](https://docs.rs/shuttle-service/0.7.1/shuttle_service/#deploying)
...
Luckily shuttle have created this boilerplate main function that will install and initialize the hello world rocket app
directly. Simpy type in the following

I open up `src/lib.rs` and overwrite it with the shuttle entrypoint code:

```
cargo shuttle init --rocket<helloworld>
```
A sample of the `cargo.toml` file it produces is as seen in the snippet below.

![](/images/url-shortner-cargo-toml.png)
...
shuttle-service = { version = "0.7.2", features = ["sqlx-postgres", "web-rocket"] }
```

My IDE quietens down as dependencies are resolved and a wave of relief washes
over me. Let's deploy this thing.

Before deploying the app we have to create a new shuttle project which is done as follows
```sh
...
$ cargo shuttle deploy
   Packaging url-shortener v0.7.2 (/private/shuttle/examples/url-shortener)
   ...
   Compiling shuttle-service v0.7.2
   Compiling url-shortener v0.7.2 (/opt/shuttle/crates/url-shortener)
...
[shuttle / sqlx](https://docs.rs/shuttle-service/0.7.1/shuttle_service/#using-sqlx)
