...
This will allow us to be able to use the `query!()` and `query_as!()` queries, which like the above allows us to execute general queries as well as queries that can directly return structs. The main difference however is that these are statically checked at compile-time, so they'll always be syntactically correct at runtime. Let's have a look at how we can use these below:
