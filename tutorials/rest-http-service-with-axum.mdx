...
However, what if we want to make our own cookies? Suppose we have a backend that uses CORS and requires secure, same-site cookies - what would we do? In this case, we can declare a variable as a cookie using Axum's `build` method for the Cookie struct. Let's see what building this might look like:
...
req: Request<B>,
next: Next<B>) -> Result<Response, StatusCode> {
...
### Deployment
Normally, you'd need to bind your router to an IP address with port (making it into a service) to be able to run it:
...
However, with Shuttle you can wrap your entry point function with the Shuttle runtime macro and then add relevant code annotations, and it'll work without needing to do anything else. You can also then deploy your app with `cargo shuttle deploy` and then it'll deploy your app to a live URL (assuming there's no errors). No Docker containerisation required!