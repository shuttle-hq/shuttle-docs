...
serde_json = "1.0"
shuttle-axum = { version = "0.34.0" }
shuttle-runtime = { version = "0.34.0" }
shuttle-secrets = "0.34.0"
shuttle-static-folder = "0.34.0"
sync_wrapper = "0.1"
...
```rust
    ...
    // return a new router and nest the admin route into the websocket route
     Router::new()
        .route("/ws", get(ws_handler))
        .nest("/admin", admin)
        .fallback_service("/", ServeDir::new(static_folder).not_found_service(ServeFile::new(static_folder.join("/index.html"))))
        .layer(Extension(users))
}
```
