...
This guide is for transferring old projects from **shuttle.rs** to the new **shuttle.dev** platform.
...
⚠️ If your project depends on static assets or other files at runtime, you need to declare them in `build.assets` to have them copied from the builder to the runtime image ([docs](/docs/files#build-assets)).
