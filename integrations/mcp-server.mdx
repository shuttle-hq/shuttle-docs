...
Before we dive in, you'll need to set up Rust's nightly toolchain since the Shuttle MCP server uses some cutting-edge features:
...
Once you have nightly set up, installing the Shuttle MCP server is straightforward:

```sh
cargo install shuttle-mcp
```

That's it! The server is now ready to connect to your preferred AI coding environment.
