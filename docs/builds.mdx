...
## Build architecture
Unless you are on the Growth tier, your projects will primarily be built and run on an Arm64 (aarch64) architecture.
...
The Rust version in the image is regularly updated to the latest stable `stable-x86_64-unknown-linux-gnu` toolchain or the `stable-aarch64-unknown-linux-gnu` depending on your project's build architecture.
