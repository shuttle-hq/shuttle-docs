...
## Usage
Add `shuttle-persist` to the dependencies for your service with `cargo add shuttle-persist`. You can get this resource using the `shuttle-persist::Persist` attribute to get a `PersistInstance`. Objects can now be saved using `PersistInstance.save()` and loaded again using `PersistInstance.load()`.
