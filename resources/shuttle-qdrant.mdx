...
| api_key    | &str   | If the database requires it | API key for authenticating against the database. |
| local_addr | &str   | No | If secified, connect to this URL on local runs instead of using a Docker container. |
