...
This example shows how you can make a custom resource called `pdo` which outputs the value that you pass into the "name" attribute for the resource. 

We are using the Axum framework in `main.rs` so we can showcase the resource in action, but the implementation doesn't require any particular framework.  
