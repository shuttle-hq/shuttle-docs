...
* Shuttle connects by default to port 8000 - if you're currently already using something at port 8000, you can add the `--port <port-number>` to the `cargo shuttle run` command to change this.### Introduction
We've created a SaaS template that you can use to bootstrap your apps faster & quicker in JavaScript while using the power of Rust.
