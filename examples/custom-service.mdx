...
We can do do this by simply adding an implementation for our wrapper struct containing a function called `start`,
which will contain all of our tasks that we want to run and simply run them all together in one `tokio::select!` macro
(currently, this code doesn't work due to no base implementation - we will implement this later on):
...
impl CustomService {
    async fn start(self, addr: std::net::SocketAddr) ->
         Result<(), Box<dyn std::error::Error>> {
        let router = self.router.into_inner();

        let serve_router = axum::Server::bind(&addr)
