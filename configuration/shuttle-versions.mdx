...
3. Test that your project works with `cargo check` / `cargo build` / `shuttle run`.

4. Upgrade your project container with `shuttle project restart` (remember to use `--idle-minutes` if relevant). This will not delete any databases, and you will keep your project name.

5. Finally, redeploy your Shuttle app: `shuttle deploy`. *(if you want to, commit your changes first)*
