...
When you run `shuttle deploy`, all source files in your cargo workspace that are not ignored by `.gitignore` or `.ignore` rules are packed into a tar-gzip archive and uploaded to Shuttle, where they are then extracted, built, and hosted.
...
```bash
shuttle --debug deploy
```

or inspect the archive after writing it to disk:

```bash
shuttle deploy --output-archive deployment.tar
```
...
Files saved in other parts of the filesystem, such as `/tmp`, will be cleared when the container restarts, which is done when you run `shuttle project restart`.
