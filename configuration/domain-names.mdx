...
If you have your domain name on Cloudflare, the process is quite simple:

Go to `Websites -> <your domain> -> DNS -> Records -> Add Record`.

---

If you are adding a root-level domain, add a `CNAME` record from `@` to your `.shuttleapp.rs` subdomain. Disable the Proxy.
...
If you are adding a subdomain, add a `CNAME` record from your subdomain to your `.shuttleapp.rs` subdomain. Disable the Proxy.
