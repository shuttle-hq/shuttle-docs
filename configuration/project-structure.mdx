...
Shuttle does not allow explicitly setting feature flags (yet), but instead activates the feature `shuttle` if it exists (along with disabling default features).
