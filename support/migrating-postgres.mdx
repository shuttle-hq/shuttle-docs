---
title: "Migrating from Shuttle RDS PostgreSQL to Shuttle Shared PostgreSQL"
---
...
[Shuttle shared DB](./shuttle-shared-db) to provision a shared PostgreSQL database for
your project. You will now have two `pool`s in your main function, you can ignore the
...
databases, we'll need these to dump the RDS and restore it in the shared database.

5. You can use the `psql` CLI to run some queries against the RDS database before dumping,
so you can run the same queries against the shared database after completing the migration.
You can use this comparison to verify the migration was complete.
...
the RDS user as the owner of your tables with the username from your shared database connection string.
...
pool, so our app will use that going forward. 
