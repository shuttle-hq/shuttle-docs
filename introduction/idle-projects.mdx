...
API endpoint for an axum server, or calling the commands of a discord
bot in a discord server). An HTTP server (eg. axum or rocket), will need about 2 requests per minute over the idle-time period to be considered
...
creating a project: `cargo shuttle project new --idle-minutes 40`. If you pass in `--idle-minutes 0`
