...
The global subscriber has an `env_filter` which defaults to the `DEBUG` log level. You can change the log level for local runs
with `RUST_LOG="<crate-name>=trace" cargo shuttle run`, but we do not currently allow changing the log level for production deployments.
